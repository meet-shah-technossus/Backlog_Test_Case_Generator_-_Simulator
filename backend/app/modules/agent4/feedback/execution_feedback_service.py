...
class Agent4ExecutionFeedbackService:
    """Validates and interprets Agent5 execution feedback for Agent4 Phase 8."""

    _OUTCOMES = {"passed", "partial", "failed"}
    _RECOMMENDED_ACTIONS = {"none", "retry_generation", "manual_review", "abort"}
    _MAX_STEP_RESULTS = 2000
    ...
    def validate_feedback_request(
        ...
        *,
        ...
        for item in step_results:
            ...
            if not str(item.get("script_path") or item.get("step_id") or "").strip():
                raise ValueError("Each step_results item requires script_path or step_id")
    ...
    def derive_transition(
        ...
        *,
        ...
        if recommended_action == "abort":
            return {
                "state": "failed",
                "stage": "phase-8-feedback-aborted",
                "last_error_code": "A4_EXECUTION_ABORTED",
                "last_error_message": "Execution feedback requested abort",
                "action": "execution_feedback_abort",
            }

        if recommended_action == "retry_generation":
            return {
                "state": "generation_ready",
                "stage": "phase-8-feedback-retry-requested",
                "last_error_code": "A4_EXECUTION_RETRY_REQUESTED",
                "last_error_message": "Execution feedback requested script regeneration",
        ...
        if outcome == "passed" and failed_steps == 0:
            return {
                ...
                "stage": "phase-8-feedback-accepted",
        ...
        return {
            ...
            "stage": "phase-8-feedback-review-required",
            "last_error_code": "A4_EXECUTION_FEEDBACK_REVIEW_REQUIRED",
