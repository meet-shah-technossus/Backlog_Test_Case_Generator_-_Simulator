...
def _run_to_handoff_emitted(container: AppContainer, step_action: str, gate_mode: str) -> str:
...
def main() -> None:
    ...
    run_pass = _run_to_handoff_emitted(container, "Type wireless mouse in search input", "quick")
    accepted = orchestrator.apply_phase6_feedback(
        run_id=run_pass,
    ...
    print("PASS_RUN", run_pass)
    print("PASS_STATE", (accepted.get("run") or {}).get("state"))
    print("PASS_STAGE", (accepted.get("run") or {}).get("stage"))

    run_retry = _run_to_handoff_emitted(container, "Type wireless mouse in search input", "quick")
    retry = orchestrator.apply_phase6_feedback(
        run_id=run_retry,
        message_id=f"a4-feedback-{uuid4().hex[:8]}",
        source_agent4_run_id=f"agent4-run-{uuid4().hex[:8]}",
        outcome="failed",
        recommended_action="retry_selectors",
        step_results=[
            {"step_id": "TC001-S1", "status": "failed", "error": "selector timeout"},
            {"step_id": "TC001-S2", "status": "failed", "error": "no element"},
        ],
        summary={"browser": "chromium"},
        metadata={"env": "smoke"},
    )
    print("RETRY_RUN", run_retry)
    print("RETRY_STATE", (retry.get("run") or {}).get("state"))
    print("RETRY_STAGE", (retry.get("run") or {}).get("stage"))
    print("RETRY_ERROR", (retry.get("run") or {}).get("last_error_code"))
